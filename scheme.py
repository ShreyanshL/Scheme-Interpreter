...
def eval_all_tail(expressions, env):
    """Evaluate each expression in the Scheme list EXPRESSIONS in
    environment ENV and return the value of the last."""
    # BEGIN PROBLEM 8
    while expressions is not nil:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Wrong Number of Arguments given!")
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lam = LambdaProcedure(target.second, expressions.second,env)
        env.define(target.first,lam)
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 1 and scheme_truep(curr_val):
        return curr_val
    elif scheme_falsep(curr_val):
        return False
    else:
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 1 and scheme_falsep(curr_val):
        return False
    elif scheme_truep(curr_val):
        return curr_val
    else:
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil:
                return test
            consequent = eval_all(clause.second,env)
            return consequent
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(curr,2,2)
        formals = curr.first 
        if self_evaluating(formals):
            raise SchemeError("Formals cannot be a number")
        value = scheme_eval(curr.second.first,env)
        child.define(formals,value)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        macro = MacroProcedure(target.second, expressions.second,env)
